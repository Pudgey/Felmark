...
const BLOCK_LABEL_COLORS: Record<string, string> = {
  ...
  h2: "var(--ink-500)",
  h3: "var(--ink-400)",
  paragraph: "var(--ink-300)",
  todo: "#5a9a3c",
  callout: "var(--ember)",
  divider: "var(--warm-300)",
  code: "#5b7fa4",
  bullet: "var(--ink-400)",
  numbered: "var(--ink-400)",
  quote: "var(--ink-400)",
  graph: "#5b7fa4",
  deliverable: "#5b7fa4",
  money: "#5a9a3c",
  table: "var(--ink-500)",
  accordion: "var(--ink-500)",
  math: "var(--ember)",
  image: "#5b7fa4",
  gallery: "#5b7fa4",
  swatches: "var(--ember)",
  beforeafter: "var(--ink-500)",
  bookmark: "#5b7fa4",
  deadline: "#c24b38",
  audio: "#8a7e63",
  ai: "#7864b4",
  ...
  timeline: "#5b7fa4",
  flow: "#7c6b9e",
  brandboard: "var(--ember)",
  moodboard: "#8a7e63",
  wireframe: "#7c8594",
  pullquote: "var(--ember)",
  "hero-spotlight": "var(--ember)",
  "kinetic-type": "var(--ink-500)",
  "number-cascade": "var(--ember)",
  "stat-reveal": "#5a9a3c",
  "value-counter": "var(--ember)",
  "pricing-config": "var(--ember)",
  "scope-boundary": "#7c8594",
  "asset-checklist": "#5a9a3c",
  "decision-picker": "var(--ember)",
  "availability-picker": "#5b7fa4",
  "progress-stream": "var(--ink-500)",
  "dependency-map": "#7c8594",
  "revision-heatmap": "var(--ember)",
};
...
function getBlockSummary(block: Block) {
  switch (block.type) {
    ...
    case "graph":
      return firstText(
        block.graphData?.title,
        block.graphData?.graphType ? `${humanizeToken(block.graphData.graphType)} chart` : "",
      );
    case "deliverable":
    ...
    case "accordion":
      return firstText(
        block.accordionData?.items?.[0]?.title,
        block.accordionData?.items?.length ? countLabel(block.accordionData.items.length, "section") : "",
        "accordion",
      );
    case "math":
    ...
    case "image":
      return firstText(
        ...
        block.imageData?.alt,
        block.imageData?.src ? "image added" : "",
        "image",
      );
    case "gallery":
    ...
    case "mention":
      return firstText(
        block.mentionData?.person ? `mention ${block.mentionData.person}` : "",
        block.mentionData?.message,
        "mention",
      );
    case "question":
    ...
    case "feedback":
      return firstText(
        block.feedbackData?.description,
        block.feedbackData?.reviewer ? `feedback from ${block.feedbackData.reviewer}` : "",
        "feedback",
      );
    case "decision":
    ...
    case "poll":
      return firstText(
        block.pollData?.question,
        block.pollData?.options?.length ? countLabel(block.pollData.options.length, "option") : "",
        "poll",
      );
    case "handoff":
      return firstText(
        block.handoffData?.notes,
        block.handoffData?.from && block.handoffData?.to ? `${block.handoffData.from} → ${block.handoffData.to}` : "",
        "handoff",
      );
    case "signoff":
    ...
    case "annotation":
      return firstText(
        block.annotationData?.pins?.[0]?.comment,
        block.annotationData?.pins?.length ? countLabel(block.annotationData.pins.length, "annotation") : "",
        "annotation",
      );
    case "canvas":
      return firstText(
    ...
    case "drawing":
      return firstText(
        block.drawingData?.title,
        block.drawingData?.drawingType ? humanizeToken(block.drawingData.drawingType) : "",
        "drawing",
      );
    case "ai-action":
      return firstText(
        block.aiActionData?.targetLabel,
        block.aiActionData?.output,
        block.aiActionData?.mode ? `${humanizeToken(block.aiActionData.mode)} action` : "",
        "AI action",
      );
    case "timeline":
    ...
    case "pricing-config":
      return block.pricingConfigData
        ? `${block.pricingConfigData.selected.length}/${block.pricingConfigData.options.length} selected`
        : "pricing config";
    case "scope-boundary":
      return firstText(
        block.scopeBoundaryData?.note,
        block.scopeBoundaryData ? `${countLabel(block.scopeBoundaryData.inScope.length, "scope item")}` : "",
        "scope boundary",
      );
    case "asset-checklist":
      return block.assetChecklistData?.items?.length
        ? countLabel(block.assetChecklistData.items.length, "asset")
        : "asset checklist";
    case "decision-picker": {
      const picked = block.decisionPickerData?.options.find((option) => option.id === block.decisionPickerData?.choice);
      return firstText(
        picked?.label,
        block.decisionPickerData?.options?.length ? countLabel(block.decisionPickerData.options.length, "option") : "",
        "decision picker",
      );
    }
    case "availability-picker":
      return firstText(
        block.availabilityPickerData?.selected,
        block.availabilityPickerData?.days?.length ? countLabel(block.availabilityPickerData.days.length, "day") : "",
        "availability",
      );
    case "progress-stream":
      return firstText(
        block.progressStreamData?.snapshots?.[0]?.label,
        block.progressStreamData?.snapshots?.length
          ? countLabel(block.progressStreamData.snapshots.length, "update")
          : "",
        "progress stream",
      );
    case "dependency-map":
      return firstText(
        block.dependencyMapData?.nodes?.[0]?.label,
        block.dependencyMapData?.nodes?.length ? countLabel(block.dependencyMapData.nodes.length, "dependency") : "",
        "dependency map",
      );
    case "revision-heatmap":
      return firstText(
        block.revisionHeatmapData?.sections?.[0]?.name,
        block.revisionHeatmapData?.sections?.length
          ? countLabel(block.revisionHeatmapData.sections.length, "section")
          : "",
        "revision heatmap",
      );
    default:
...
export default function EditorMargin({
  ...
  const handleItemClick = useCallback(
    (e: React.MouseEvent, block: Block, index: number) => {
          ...
          {blocks.map((block, i) => {
            ...
            return (
              <div
                ...
                onDragOver={(e) => {
                  e.preventDefault();
                  if (dragRef.current !== null && dragRef.current !== i) setDropIdx(i);
                }}
                onDragLeave={() => {
        ...
        <div className={styles.selectionBar}>
          ...
          <button className={styles.selectionBtn} title="Copy (⌘C)" onClick={handleCopy}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              ...
              <path
                d="M10 4V3a1.5 1.5 0 00-1.5-1.5H3A1.5 1.5 0 001.5 3v5.5A1.5 1.5 0 003 10h1"
                stroke="currentColor"
                strokeWidth="1.2"
              />
            </svg>
          ...
          {onDeleteBlocks && (
            ...
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path
                  d="M3 4h8M5.5 4V3a1 1 0 011-1h1a1 1 0 011 1v1M5.5 6.5v3.5M8.5 6.5v3.5"
                  stroke="currentColor"
                  strokeWidth="1.2"
                  strokeLinecap="round"
                />
                <path d="M3.5 4l.5 7.5a1 1 0 001 1h4a1 1 0 001-1L10.5 4" stroke="currentColor" strokeWidth="1.2" />
              </svg>
            </button>
          )}
        ...
        >
          ...
          {onDeleteBlocks && (
            <button
              ...
              onClick={() => {
                onDeleteBlocks([ctxMenu.blockId]);
                setCtxMenu(null);
              }}
            >
              Delete block
