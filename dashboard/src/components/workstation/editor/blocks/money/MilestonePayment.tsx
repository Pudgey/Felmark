...
export interface MilestoneItem {
  ...
  deliverable: string;
  amount: number;
  status: "paid" | "invoiced" | "ready" | "pending";
...
const STATUS_ICONS: Record<string, { icon: string; color: string }> = {
  paid: { icon: "\u2713", color: "var(--success)" },
  invoiced: { icon: "\u2197", color: "var(--info)" },
  ready: { icon: "\u25cf", color: "var(--ember)" },
  ...
  const triggerInvoice = (id: string) => {
    onUpdate?.({
      milestones: milestones.map((m) =>
        m.id === id ? { ...m, status: "invoiced" as const, invoiceNum: `#${48 + milestones.indexOf(m)}` } : m,
      ),
    });
  };
  ...
  return (
    <div className={styles.mb}>
      <div className={styles.head}>
        <span className={styles.icon} style={{ color: "var(--success)" }}>
          &#x2691;
        </span>
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity: m.status === "pending" ? 0.15 : m.status === "ready" ? 0.4 : 0.7,
              }}
            />
          );
        })}
        ...
        {milestones.map((m, i) => {
          ...
          return (
            <div key={m.id} className={`${styles.mileRow}${m.status === "ready" ? ` ${styles.mileRowReady}` : ""}`}>
              ...
              <div className={styles.mileIcon} style={{ color: cfg.color }}>
                {cfg.icon}
              </div>
              ...
              <span
                className={styles.mileAmt}
                style={{ color: m.status === "paid" ? "var(--success)" : "var(--ink-700)" }}
              >
                ${m.amount.toLocaleString()}
              </span>
              {m.status === "ready" && (
                <button className={styles.mileTrigger} onClick={() => triggerInvoice(m.id)}>
                  Send Invoice
                </button>
              )}
              {m.status === "paid" && <span className={`${styles.mileBadge} ${styles.mileBadgePaid}`}>Paid</span>}
              {m.status === "invoiced" && (
                <span className={`${styles.mileBadge} ${styles.mileBadgeInv}`}>Invoiced</span>
              )}
            </div>
          );
        })}
      ...
      <div className={styles.mileSummary}>
        <span className={styles.mileSumItem}>
          <span className={styles.mileSumDot} style={{ background: "var(--success)" }} /> ${paidTotal.toLocaleString()}{" "}
          received
        </span>
        {readyTotal > 0 && (
          <span className={styles.mileSumItem}>
            <span className={styles.mileSumDot} style={{ background: "var(--ember)" }} /> ${readyTotal.toLocaleString()}{" "}
            ready
          </span>
        )}
        <span className={styles.mileSumItem}>
          <span className={styles.mileSumDot} style={{ background: "var(--warm-300)" }} /> $
          {(total - paidTotal - readyTotal).toLocaleString()} remaining
        </span>
      </div>
    </div>
  );
