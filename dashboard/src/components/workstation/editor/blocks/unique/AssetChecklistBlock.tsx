...
const STATUS_CFG: Record<string, { color: string; icon: string; label: string }> = {
  received: { color: "var(--success)", icon: "✓", label: "Received" },
  partial: { color: "var(--ember-light)", icon: "◐", label: "Partial" },
  missing: { color: "var(--error)", icon: "!", label: "Missing" },
  "not-needed": { color: "var(--ink-400)", icon: "—", label: "Not needed" },
};
    ...
    items: [
      {
        ...
        desc: "SVG, EPS, or AI format preferred",
        status: "received",
        receivedDate: "Mar 18",
        fileType: "SVG",
      },
      {
        id: "a2",
        name: "Brand colors (if existing)",
        desc: "Hex codes or Pantone references",
        status: "received",
        receivedDate: "Mar 18",
        fileType: "PDF",
      },
      {
        ...
        desc: "3–5 competitors for analysis",
        status: "received",
        receivedDate: "Mar 20",
        fileType: "Link",
      },
      {
        ...
        desc: "Team photos, office, products",
        status: "partial",
        note: "3 of 8 uploaded",
      },
      { id: "a5", name: "Content copy", desc: "Homepage, about, and services text", status: "missing", daysWaiting: 5 },
      {
        id: "a6",
        name: "Social media access",
        desc: "Admin access to IG, LinkedIn, X",
        status: "missing",
        daysWaiting: 5,
      },
      {
        id: "a7",
        name: "Target audience brief",
        desc: "Demographics, psychographics, personas",
        status: "not-needed",
        note: "We'll create this in discovery",
      },
    ],
  };
}
    ...
    <div className={styles.card}>
      <div className={styles.header}>
        <span
          ...
          style={{ color: "#8a7e63", background: "rgba(138,126,99,0.06)", borderColor: "rgba(138,126,99,0.1)" }}
        >
          Assets
        </span>
      ...
      <div className={styles.assetsList}>
        {data.items.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`${styles.assetItem} ${item.status === "missing" ? styles.assetMissing : ""}`}
            >
              <div
                className={styles.assetStatus}
                style={{ color: st.color, background: st.color + "08", borderColor: st.color + "15" }}
              ...
              <div className={styles.assetRight}>
                {item.status === "received" && (
                  <span className={`${styles.assetTag} ${styles.assetTagReceived}`}>
                    {item.fileType} · {item.receivedDate}
                  </span>
                )}
                {item.status === "partial" && (
                  <span className={`${styles.assetTag} ${styles.assetTagPartial}`}>{item.note}</span>
                )}
                {item.status === "missing" && (
                  <span className={`${styles.assetTag} ${styles.assetTagMissing}`}>{item.daysWaiting}d waiting</span>
                )}
                {item.status === "not-needed" && (
                  <span className={`${styles.assetTag} ${styles.assetTagNotNeeded}`}>{item.note}</span>
                )}
              </div>
            </div>
          );
      ...
      {missingCount > 0 && (
        <div className={styles.assetsNudge}>
          ...
          <span>
            {missingCount} item{missingCount > 1 ? "s are" : " is"} blocking progress.
          </span>
