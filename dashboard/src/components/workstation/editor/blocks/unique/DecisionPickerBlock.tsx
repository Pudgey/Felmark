    ...
    options: [
      {
        id: "a",
        label: "Direction A",
        subtitle: "Organic & Warm",
        desc: "Earth tones, serif typography, hand-drawn textures. Feels artisanal, approachable, and premium.",
      ...
      },
      {
        ...
        desc: "Monochrome with accent, geometric sans-serif, sharp lines. Professional, scalable, tech-forward.",
      ...
      },
      {
        ...
        desc: "High contrast, oversized type, dynamic color palette. Energetic, confident, impossible to ignore.",
    ...
    <div className={styles.card}>
      <div className={styles.header}>
        <span
          ...
          style={{ color: "var(--muted)", background: "rgba(124,107,158,0.06)", borderColor: "rgba(124,107,158,0.1)" }}
        >
          Decision
        </span>
      ...
      <div className={styles.decisionGrid}>
        {data.options.map((opt) => (
          <div
            key={opt.id}
            className={`${styles.decisionCard} ${data.choice === opt.id ? styles.decisionChosen : ""}`}
          ...
          >
            ...
            <div className={styles.decisionColors}>
              {opt.colors.map((c, i) => (
                <div
                  key={i}
                  className={styles.decisionColor}
                  style={{
                    background: c,
                    border: ["#fafafa", "var(--parchment)", "#ffffff", "#e5e5e5"].includes(c)
                      ? "1px solid #e5e2db"
                      : "none",
                  }}
                />
            ...
            <div
              className={styles.decisionFont}
              style={{ fontFamily: opt.font === "Cormorant Garamond" ? "'Cormorant Garamond', serif" : "inherit" }}
            >
              {opt.font}
            </div>
            ...
            <div className={styles.decisionPros}>
              {opt.pros.map((p, i) => (
                <span key={i} className={styles.decisionPro}>
                  ✓ {p}
                </span>
              ))}
            </div>
      ...
      {chosen && (
        <div className={styles.decisionConfirm}>
          You selected <strong>{chosen.label}</strong> — {chosen.subtitle} · ${chosen.price.toLocaleString()} ·{" "}
          {chosen.timeline}
        </div>
      )}
    </div>
