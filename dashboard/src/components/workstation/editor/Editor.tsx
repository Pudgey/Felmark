...
import GraphBlockComponent, { getDefaultGraphData, GRAPH_TYPE_OPTIONS } from "./blocks/graphs/GraphBlock";
import GraphDataEditor from "./blocks/graphs/GraphDataEditor";
import graphStyles from "./blocks/graphs/GraphBlock.module.css";
import MoneyBlockComponent, { getDefaultMoneyData, MONEY_TYPE_OPTIONS } from "./blocks/money/MoneyBlock";
...
import DeadlineBlockComponent, { getDefaultDeadlineData } from "./blocks/deadline/DeadlineBlock";
...
import CommentThreadBlock, { getDefaultCommentThread } from "./blocks/comment-thread/CommentThreadBlock";
import MentionBlock, { getDefaultMention } from "./blocks/mention/MentionBlock";
import QuestionBlock, { getDefaultQuestion } from "./blocks/question/QuestionBlock";
import FeedbackBlock, { getDefaultFeedback } from "./blocks/feedback/FeedbackBlock";
import DecisionBlock, { getDefaultDecision } from "./blocks/decision/DecisionBlock";
import PollBlock, { getDefaultPoll } from "./blocks/poll/PollBlock";
import HandoffBlock, { getDefaultHandoff } from "./blocks/handoff/HandoffBlock";
import SignoffBlock, { getDefaultSignoff } from "./blocks/signoff/SignoffBlock";
import AnnotationBlock, { getDefaultAnnotation } from "./blocks/annotation/AnnotationBlock";
import AiActionBlock, { getDefaultAiActionData } from "./blocks/ai-action/AiActionBlock";
import TimelineBlock, { getDefaultTimeline } from "./blocks/timeline/TimelineBlock";
import FlowBlock, { getDefaultFlow } from "./blocks/flow/FlowBlock";
import BrandBoardBlock, { getDefaultBrandBoard } from "./blocks/brand-board/BrandBoardBlock";
import MoodBoardBlock, { getDefaultMoodBoard } from "./blocks/mood-board/MoodBoardBlock";
import WireframeBlock, { getDefaultWireframe } from "./blocks/wireframe/WireframeBlock";
import PullQuoteBlock, { getDefaultPullQuote } from "./blocks/pull-quote/PullQuoteBlock";
import HeroSpotlightBlock, { getDefaultHeroSpotlight } from "./blocks/hero-spotlight/HeroSpotlightBlock";
import KineticTypeBlock, { getDefaultKineticType } from "./blocks/kinetic-type/KineticTypeBlock";
import NumberCascadeBlock, { getDefaultNumberCascade } from "./blocks/number-cascade/NumberCascadeBlock";
import StatRevealBlock, { getDefaultStatReveal } from "./blocks/stat-reveal/StatRevealBlock";
import ValueCounterBlock, { getDefaultValueCounter } from "./blocks/value-counter/ValueCounterBlock";
import PricingConfigBlock, { getDefaultPricingConfigData } from "./blocks/unique/PricingConfigBlock";
import ScopeBoundaryBlock, { getDefaultScopeBoundaryData } from "./blocks/unique/ScopeBoundaryBlock";
import AssetChecklistBlock, { getDefaultAssetChecklistData } from "./blocks/unique/AssetChecklistBlock";
import DecisionPickerBlock, { getDefaultDecisionPickerData } from "./blocks/unique/DecisionPickerBlock";
import AvailabilityPickerBlock, { getDefaultAvailabilityPickerData } from "./blocks/unique/AvailabilityPickerBlock";
import ProgressStreamBlock, { getDefaultProgressStreamData } from "./blocks/unique/ProgressStreamBlock";
import DependencyMapBlock, { getDefaultDependencyMapData } from "./blocks/unique/DependencyMapBlock";
import RevisionHeatmapBlock, { getDefaultRevisionHeatmapData } from "./blocks/unique/RevisionHeatmapBlock";
...
export default function Editor({ workstations, tabs, activeProject, blocks: blocksProp, sidebarOpen, wordCount, charCount, onOpenSidebar, onTabClick, onTabClose, onNewTab, onTabRename, onBlocksChange, onWordCountChange, activeWorkstationId, onSelectProject, onSelectWorkstationHome, onSaveAsTemplate, docTemplates, onNavigateRail, onRenameWorkstation, onUpdateProjectDue, comments, onCommentsChange, activities, onActivitiesChange, zenMode, onToggleZen, splitProject, splitBlocks, splitProjectName, splitClientName, onSplitOpen, onSplitClose, onSplitMakePrimary }: EditorProps) {
  ...
  const setBlocks = useCallback((updater: Block[] | ((prev: Block[]) => Block[])) => {
    ...
    settleSaveState();
  }, [commitBlocks, mergeCachedContent, settleSaveState]);
  ...
  const onContentChange = useCallback((id: string, html: string, text: string) => {
    ...
    setSaveState("saving");
    if (typingSaveTimer.current) clearTimeout(typingSaveTimer.current);
    typingSaveTimer.current = setTimeout(() => flushCachedContent(), 550);
  ...
  }, [activeProject, blocks, emitWordCounts, flushCachedContent, tabs, onTabRename]);
  ...
  const duplicateBlockById = useCallback((sourceId: string) => {
    ...
    focusNew(cloned.id);
    return true;
  }, [blocks, focusNew, pushUndoAction, setBlocks, snapshotCurrentBlocks]);
  ...
  const selectSlashItem = useCallback((type: BlockType) => {
    ...
    setSlashMenu(null);
  }, [pushUndoAction, setBlocks, slashMenu, snapshotCurrentBlocks, focusNew]);
  ...
  const restoreUndoSnapshot = useCallback(() => {
    ...
    setSaveState("saving");
    settleSaveState();
    if (undoAction.focusId) setTimeout(() => focusNew(undoAction.focusId!), 20);
  }, [activeProject, emitWordCounts, focusNew, onBlocksChange, restoreContentCache, settleSaveState, undoAction]);
