      ...
      if (hitId !== null) {
        ...
        if (shiftKey) {
          ...
          if (newSelection.has(hitId)) {
            ...
            // Shift-clicked to deselect — no drag
            return;
          }
          newSelection.add(hitId);
        } else if (selectedIds.has(hitId)) {
          // Already selected — keep entire selection for group drag
          newSelection = new Set(selectedIds);
        } else {
          // Click unselected element — select only it
          newSelection = new Set([hitId]);
        }
        setSelectedIds(newSelection);

        // Build originals snapshot for all elements we'll move
