...
export default function Canvas() {
  ...
  const handleCanvasMove = (e) => {
    ...
    if (dragging && dragTypeRef.current) {
      const def = BLOCK_TYPES.find(bt => bt.type === dragTypeRef.current);
      if (def) setGhostPos({ x: Math.min(cell.col, COLS - def.defaultW), y: cell.row, w: def.defaultW, h: def.defaultH });
    }
  };

  const handleCanvasDrop = (e) => {
    if (!dragging || !dragTypeRef.current) return;
    const cell = canvasToGrid(e.clientX, e.clientY);
    if (!cell) return;
    const def = BLOCK_TYPES.find(bt => bt.type === dragTypeRef.current);
    if (!def) return;
    const x = Math.min(cell.col, COLS - def.defaultW);
    const y = cell.row;
    setBlocks(prev => [...prev, { id: `b${Date.now()}`, x, y, w: def.defaultW, h: def.defaultH, type: def.type, label: def.label, color: def.color }]);
    setDragging(false);
  ...
  // Global mousemove/mouseup so dragging works even outside the canvas/library
  useEffect(() => {
    if (!dragging) return;
    const onMove = (e) => {
      const cell = canvasToGrid(e.clientX, e.clientY);
      if (!cell || !dragTypeRef.current) return;
      const def = BLOCK_TYPES.find(bt => bt.type === dragTypeRef.current);
      if (def) setGhostPos({ x: Math.min(cell.col, COLS - def.defaultW), y: cell.row, w: def.defaultW, h: def.defaultH });
    };
    const onUp = (e) => {
      ...
      if (cell) {
        ...
        if (def) {
          const x = Math.min(cell.col, COLS - def.defaultW);
          setBlocks(prev => [...prev, { id: `b${Date.now()}`, x, y: cell.row, w: def.defaultW, h: def.defaultH, type: def.type, label: def.label, color: def.color }]);
        }
      }
      setDragging(false);
      dragTypeRef.current = null;
...
.cv-foot{height:28px;padding:0 20px;border-top:1px solid var(--border);background:var(--card);display:flex;align-items:center;justify-content:space-between;font-family:var(--mono);font-size:9px;color:var(--ink-300);flex-shrink:0;z-index:20}
      ...
      <div className="cv">
        ...
        <div className="cv-canvas" ref={canvasRef} onMouseMove={handleCanvasMove} style={{ cursor: dragging ? "grabbing" : "default" }}>
            ...
            {placingBlock && ghostPos && (
              <div className="cv-ghost" style={{
                ...
                <div style={{ padding: 12, fontFamily: "var(--mono)", fontSize: 10, color: "var(--lavender)" }}>
                  ...
                  <div style={{ fontSize: 8, color: "var(--ink-300)", marginTop: 2 }}>{ghostPos.w}×{ghostPos.h} · Release to place</div>
                </div>
              </div>
            )}
      ...
      {showLibrary && (
        <div className="cv-lib">
          ...
          <div className="cv-lib-list">
            {BLOCK_TYPES.map(bt => (
              <div key={bt.type} className="cv-lib-item" onMouseDown={(e) => { e.preventDefault(); startDrag(bt.type); }} style={{ cursor: "grab" }}>
      ...
      {dragging && placingBlock && (
        <div className="cv-placing">
          ...
          <span style={{ fontFamily: "var(--mono)", fontSize: 10, opacity: .3 }}>Drop on grid to place</span>
        </div>
      )}
    </>
  );
